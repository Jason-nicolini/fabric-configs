...
```
→ Re-verify connection is alive (if >10 tool calls since last check)
→ List objects to translate for this phase
→ Batch translate using batch_object_translation_operations
→ Verify: spot-check 2-3 translations with get operations
```
**Checkpoint after each phase:** "Phase N complete: translated X of Y [object type]. Verified."
